...
-- -----------------------------------------------------
-- Table `vn`.`toimintaalue`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `vn`.`toimintaalue` ;

CREATE TABLE IF NOT EXISTS `vn`.`toimintaalue` (
  `toimintaalue_id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `nimi` VARCHAR(40) NULL DEFAULT NULL,
  PRIMARY KEY (`toimintaalue_id`),
  INDEX `Toimintaalue_nimi_index` (`nimi` ASC))
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
...
CREATE TABLE IF NOT EXISTS `vn`.`mokki` (
  ...
  INDEX `fk_mokki_alue_idx` (`toimintaalue_id` ASC),
  INDEX `fk_mokki_posti_idx` (`postinro` ASC),
  CONSTRAINT `fk_mokki_toimintaalue`
    FOREIGN KEY (`toimintaalue_id`)
    REFERENCES `vn`.`toimintaalue` (`toimintaalue_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `vn`.`palvelu` (
  ...
  INDEX `palv_toimip_id_ind` (`toimintaalue_id` ASC),
